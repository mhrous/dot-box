...
class Border {
  ...
  bool isFreeMove(String id) {
    List array = id.split("_");

    var first = int.tryParse(array[0]);
    var secand = int.tryParse(array[1]);

    if (first != null && box[first] > 1) return false;

    if (secand != null && box[secand] > 1) return false;
  ...
  List getAllFreeMove() {
    List freeMove = [];
    for (String m in move) {
      if (isFreeMove(m)) freeMove.add(m);
    }
